...
import type { Summary, ProduceVariety, Market, OverallSummary } from "@shared/schema";
...
export default function Dashboard() {
  ...
  const { data: varieties } = useQuery<ProduceVariety[]>({
  ...
  const { data: overallSummary } = useQuery<OverallSummary>({
          ...
          <Card className="bg-white rounded-xl shadow-lg">
            <CardContent className="p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
                {overallSummary.breakdown && overallSummary.breakdown.map((item) => (
                  <div 
                    key={item.category} 
